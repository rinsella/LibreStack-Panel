    ...
    public function handle(SafeOps $ops): int
    {
        ...
        try {
            match ($operation) {
                ...
                'file-write'         => $ops->fileWrite($args[0] ?? '', $args[1] ?? '', $args[2] ?? '', $this->b64($args[3] ?? '')),
                'file-upload'        => $ops->fileUpload($args[0] ?? '', $args[1] ?? '', $args[2] ?? '', $args[3] ?? ''),
                'file-create'        => $ops->fileCreate($args[0] ?? '', $args[1] ?? '', $args[2] ?? ''),
                'dir-create'         => $ops->dirCreate($args[0] ?? '', $args[1] ?? '', $args[2] ?? ''),
                'file-rename'        => $ops->fileRename($args[0] ?? '', $args[1] ?? '', $args[2] ?? '', $args[3] ?? ''),
                'file-move'          => $ops->fileMove($args[0] ?? '', $args[1] ?? '', $args[2] ?? '', $args[3] ?? ''),
                'file-delete'        => $ops->fileDelete($args[0] ?? '', $args[1] ?? '', $args[2] ?? ''),
                'file-copy'          => $ops->fileCopy($args[0] ?? '', $args[1] ?? '', $args[2] ?? '', $args[3] ?? ''),
                'file-chmod'         => $ops->fileChmod($args[0] ?? '', $args[1] ?? '', $args[2] ?? '', (int) octdec($args[3] ?? '0')),
                'file-zip'           => $ops->fileZip($args[0] ?? '', $args[1] ?? '', $args[2] ?? '', $args[3] ?? ''),
                'file-unzip'         => $ops->fileUnzip($args[0] ?? '', $args[1] ?? '', $args[2] ?? '', $args[3] ?? ''),
                'php-fpm-pool-write' => $ops->phpFpmPoolWrite($args[0] ?? '', $args[1] ?? '', $this->b64($args[2] ?? '')),
                'php-fpm-pool-delete' => $ops->phpFpmPoolDelete($args[0] ?? '', $args[1] ?? ''),
                'php-fpm-test'       => $ops->phpFpmTest($args[0] ?? ''),
                'php-fpm-reload'     => $ops->phpFpmReload($args[0] ?? ''),
